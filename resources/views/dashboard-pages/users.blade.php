            ...
            <div class="card mt-5">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <?php $users = \App\Models\User::orderBy('id' ,'desc')->paginate(10);
 foreach ($users as $u){
                        ?>
   {{-- <div class="col-md-4 text-center">
    ...
    color: #ccc;" ></i>
       <p><span class="user-name">{{$u->name}}</span>
        ...
        <div class="col-md-4 text-center">
            <a style="text-decoration: none;color: #000;" href="{{url('user-details/'.$u->id)}}">
                <div class="user-card bg-white m-2 p-4" style="display: flex; flex-direction: column; justify-content: center; height: 300px;">
                {{-- <div class="user-card bg-white m-2 p-4"> --}}
                    <p style="text-align: end; color: green;" <?php $admins = DB::select('select * from model_has_roles where role_id = 1'); foreach($admins as $admin){
                    ...
                    <p class="user-info">
                        ...
                        <span class="user-dept-name" style="font-weight: bold;" <?php $admins = DB::select('select * from model_has_roles where role_id = 1'); foreach($admins as $admin){
                            if($u->id == $admin->model_id) echo 'hidden';
                          } ?>><i>{{$u->department_id && $u->department_id!=-1 ?  \App\Models\Department::find($u->department_id)->getNameLocalized() : "?"}}</i></span><br>
